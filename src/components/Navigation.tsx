...
export default function Navigation({ searchQuery = '', setSearchQuery }: NavigationProps) {
  ...
  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-[1200px]">
        <nav className="bg-gradient-to-r from-[#09090d] to-[#0f0509] border border-white/[0.08] rounded-2xl shadow-2xl shadow-black/50 relative overflow-hidden">
          ...
          <div className="relative flex items-center gap-2 sm:gap-3">
            {/* Glow border behind right items (NO extra background) */}
            <div
              className="
    ...
    "
            ...
            {/* Discord Button (icon will show again) */}
            <a
              href={discordLink}
              target="_blank"
            ...
            <Link
              to="/apply"
              className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-[#ff9f43] hover:bg-[#ff9f43]/90 text-black font-semibold text-sm rounded-lg transition-all duration-200"
            >
              Apply
            </Link>
